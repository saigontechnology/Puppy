...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel.Aggregations
{
    ...
    ///     It dynamically builds fixed size (a.k.a. interval) buckets over the values. For example, if the documents have a
    ///     field that holds a price (numeric), we can configure this
    ///     aggregation to dynamically build buckets with interval 5 (in case of price it may represent $5). When the
    ///     aggregation executes, the price field of every document will be evaluated and will be rounded down to its closest
    ///     bucket - for example, if the price is 32 and the bucket size is 5
    ///     then the rounding will yield 30 and thus the document will "fall" into the bucket that is associated withe the key
    ///     30
    /// </summary>
