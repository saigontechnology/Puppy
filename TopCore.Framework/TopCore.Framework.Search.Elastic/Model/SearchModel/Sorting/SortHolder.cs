...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel.Sorting
{
    ...
    ///     field level, with special field name for _score to sort by score. When sorting, the relevant sorted field values
    ///     are loaded into memory. This means that per shard, there
    ///     should be enough memory to contain them. For string based types, the field sorted on should not be analyzed /
    ///     tokenized. For numeric types, if possible, it is recommended to explicitly set the type to six_hun types (like
    ///     short, integer and float).
    /// </summary>
    public class SortHolder : ISortHolder
    {
        ...
        /// <summary>
        ///     track_scores When sorting on a field, scores are not computed. By setting track_scores to true, scores will still
