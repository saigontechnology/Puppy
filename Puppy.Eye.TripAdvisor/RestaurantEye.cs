...
namespace Puppy.Eye.TripAdvisor
{
        ...
        public async Task<RestaurantDetailModel> GetDetail(string url)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    int startGeoJsonIndex = geoString.IndexOf("window.map0Div = ", StringComparison.CurrentCultureIgnoreCase) + "window.map0Div = ".Length;
                    geoString = geoString.Substring(startGeoJsonIndex, geoString.Length - startGeoJsonIndex);
                    geoString = geoString.Substring(0, geoString.IndexOf(";", StringComparison.CurrentCultureIgnoreCase));
