...
namespace Puppy.Search.Elastic.ContextAddDeleteUpdate.IndexModel.SettingsModel.Filters
{
    public class PatternCaptureTokenFilter : AnalysisFilterBase
    {
        ...
        /// <summary>
        ///     The pattern_capture token filter, unlike the pattern tokenizer, emits a token for
        ///     every capture group in the regular expression. Patterns are not anchored to the
        ///     beginning and end of the string, so each pattern can match multiple times, and
        ///     matches are allowed to overlap.
        /// </summary>
