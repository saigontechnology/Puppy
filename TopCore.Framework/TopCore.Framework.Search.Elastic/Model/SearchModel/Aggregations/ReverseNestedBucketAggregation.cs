...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel.Aggregations
{
    ...
    ///     the nested object in a nested aggregation. The reverse_nested aggregation must be defined inside a nested
    ///     aggregation. Options: path - Which defines to what nested object field should be joined back. The default is empty,
    ///     which means that it joins back to the root / main document level.
    ///     The path cannot contain a reference to a nested object field that falls outside the nested aggregation’s nested
    ///     structure a reverse_nested is in.
    /// </summary>
