...
namespace Puppy.Search.Elastic.ContextAddDeleteUpdate.IndexModel.SettingsModel.CharFilters
{
    public class PatternReplaceCharFilter : AnalysisCharFilterBase
    {
        ...
        /// <summary>
        ///     The pattern_replace char filter allows the use of a regex to manipulate the
        ///     characters in a string before analysis. The regular expression is defined using the
        ///     pattern parameter, and the replacement string can be provided using the replacement
        ///     parameter (supporting referencing the original text, as explained here).
        ///     "char_filter" : { "my_pattern":{ "type":"pattern_replace", "pattern":"sample(.*)",
