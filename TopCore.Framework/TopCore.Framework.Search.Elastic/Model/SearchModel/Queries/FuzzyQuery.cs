...
{
    /// <summary>
    ///     The fuzzy query uses similarity based on Levenshtein edit distance for string fields, and a +/- margin on numeric
    ///     and date fields. String fields The fuzzy query generates all possible matching terms that are within the maximum
    ///     edit distance specified in fuzziness and then checks the term
    ///     dictionary to find out which of those generated terms actually exist in the index.
    ///     Warning: this query can be very heavy if prefix_length and max_expansions are both set to 0. This could cause every
