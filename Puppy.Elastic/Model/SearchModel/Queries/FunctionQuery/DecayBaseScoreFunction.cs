...
namespace Puppy.Elastic.Model.SearchModel.Queries.FunctionQuery
{
    public abstract class DecayBaseScoreFunction<T> : BaseScoreFunction
    {
        ...
        public double Decay
        {
            ...
            set
            {
                if (value <= 0) throw new ElasticException("_decay must be larger than 0.0");
                if (value > 1) throw new ElasticException("_decay must be equal or smaller than 1.0");
                _decay = value;
                _decaySet = true;
            }
        }
        ...
        protected void WriteValues(ElasticJsonWriter elasticCrudJsonWriter)
        {
            ...
            JsonHelper.WriteValue("origin", _origin, elasticCrudJsonWriter);
            JsonHelper.WriteValue("offset", _offset, elasticCrudJsonWriter, _offsetSet);
            JsonHelper.WriteValue("scale", _scale, elasticCrudJsonWriter);
