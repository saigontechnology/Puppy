...
namespace TopCore.Framework.Search.Elastic.ContextAddDeleteUpdate.IndexModel.SettingsModel.Filters
{
    public class ShingleTokenFilter : AnalysisFilterBase
    {
        ...
        /// <summary>
        ///   A token filter of type shingle that constructs shingles (token n-grams) from a token stream. In other words, it creates combinations of
        ///   tokens as a single token. For example, the sentence "please divide this sentence into shingles" might be tokenized into shingles "please
        ///   divide", "divide this", "this sentence", "sentence into", and "into shingles". This filter handles position increments &gt; 1 by
        ///   inserting filler tokens (tokens with termtext "_"). It does not handle a position increment of 0.
        /// </summary>
        ...
        /// <summary>
        ///   If output_unigrams is false the output will contain the input tokens (unigrams) if no shingles are available. Note if output_unigrams is
        ///   set to true this setting has no effect. Defaults to false.
        /// </summary>
