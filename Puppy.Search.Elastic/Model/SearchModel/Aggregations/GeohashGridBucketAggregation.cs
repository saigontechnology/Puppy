...
namespace Puppy.Search.Elastic.Model.SearchModel.Aggregations
{
    ...
    ///     that represent cells in a grid. The resulting grid can be sparse and only contains cells
    ///     that have matching data. Each cell is labeled using a geohash which is of user-definable
    ///     precision. High precision geohashes have a long string length and represent cells that
    ///     cover only a small area. Low precision geohashes have a short string length and represent
    ///     cells that each cover a large area. Geohashes used in this aggregation can have a choice
    ///     of precision between 1 and 12. Warning The highest-precision geohash of length 12
    ///     produces cells that cover less than a square metre of land and so high-precision requests
    ///     can be very costly in terms of RAM and result sizes. Please see the example below on how
    ///     to first filter the aggregation to a smaller geographic area before requesting
    ///     high-levels of detail. The specified field must be of type geo_point (which can only be
    ///     set explicitly in the mappings) and it can also hold an array of geo_point fields, in
    ///     which case all points will be taken into account during aggregation.
    /// </summary>
    public class GeohashGridBucketAggregation : BaseBucketAggregation
    {
        ...
        /// <summary>
        ///     _precision The string length of the geohashes used to define cells/buckets in the
        ///     results. Defaults to 5. 1 : 5,009.4km x 4,992.6km 2 : 1,252.3km x 624.1km 3 : 156.5km
        ///     x 156km 4 : 39.1km x 19.5km 5 : 4.9km x 4.9km 6 : 1.2km x 609.4m 7 : 152.9m x 152.4m
        ///     8 : 38.2m x 19m 9 : 4.8m x 4.8m 10 : 1.2m x 59.5cm 11 : 14.9cm x 14.9cm 12 : 3.7cm x 1.9cm
        /// </summary>
