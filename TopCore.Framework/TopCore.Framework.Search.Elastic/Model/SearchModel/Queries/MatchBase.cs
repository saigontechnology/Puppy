...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel.Queries
{
    public abstract class MatchBase
    {
        ...
        /// <summary>
        ///     zero_terms_query If the analyzer used removes all tokens in a query like a stop filter does, the default behavior
        ...
        /// <summary>
        ///     cutoff_frequency range [0..1) The match query supports a cutoff_frequency that allows specifying an absolute or
        ///     relative document frequency where high frequency terms are moved into an optional subquery and are only scored if
        ///     one of the low frequency (below the cutoff) terms in the
        ///     case of an or operator or all of the low frequency terms in the case of an and operator match. This query allows
        ///     handling stopwords dynamically at runtime, is domain independent and doesn’t require on a stopword file. It prevent
        ///     scoring / iterating high frequency terms and only takes
        ///     the terms into account if a more significant / lower frequency terms match a document. Yet, if all of the query
        ///     terms are above the given cutoff_frequency the query is automatically transformed into a pure conjunction (and)
        ///     query to ensure fast execution. The cutoff_frequency can
        ///     either be relative to the number of documents in the index if in the range [0..1) or absolute if greater or equal
        ///     to 1.0.
        /// </summary>
