...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel
{
    ...
    ///   Currently the rescore API has only one implementation: the query rescorer, which uses a query to tweak the scoring. In the future,
    ///   alternative rescorers may be made available, for example, a pair-wise rescorer.
    ///   Note: the rescore phase is not executed when search_type is set to scan or count. The query rescorer executes a second query only on the
    ///         Top-K results returned by the query and post_filter phases. The number of docs which will be examined on each shard can be controlled
    ///         by the window_size parameter, which defaults to from and size. By default the scores from the original query and the rescore query are
    ///         combined linearly to produce the final _score for each document. The relative importance of the original query and of the rescore query
    ///         can be controlled with the query_weight and rescore_query_weight respectively. Both default to 1.
    /// </summary>
