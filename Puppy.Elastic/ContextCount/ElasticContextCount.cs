...
namespace Puppy.Elastic.ContextCount
{
    public class ElasticContextCount
    {
        ...
        public ElasticContextCount(ITraceProvider traceProvider, CancellationTokenSource cancellationTokenSource,
        ...
        {
            _traceProvider.Trace(TraceEventType.Verbose, "{2}: Request for ElasticContextCount: {0}, content: {1}",
                typeof(T), jsonContent, "ElasticContextCount");
            ...
            try
            {
                ...
                var elasticUrlForEntityGet = string.Format("{0}/{1}/{2}/_count", _connectionString,
                ...
                {
                    _traceProvider.Trace(TraceEventType.Warning, "{2}: GetCountAsync response status code: {0}, {1}",
                        response.StatusCode, response.ReasonPhrase, "ElasticContextCount");
                    if (response.StatusCode == HttpStatusCode.BadRequest)
                    {
                        ...
                        if (errorInfo.Contains("RoutingMissingException"))
                            throw new ElasticException(
                                "ElasticContextCount: HttpStatusCode.BadRequest: RoutingMissingException, adding the parent Id if this is a child item...");
            ...
            catch (OperationCanceledException oex)
            {
                _traceProvider.Trace(TraceEventType.Verbose, oex, "{1}: Get Request OperationCanceledException: {0}",
                    oex.Message, "ElasticContextCount");
                return resultDetails;
            }
        }
