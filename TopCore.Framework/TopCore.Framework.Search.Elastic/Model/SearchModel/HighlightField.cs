...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel
{
    public class HighlightField
    {
        ...
        /// <summary>
        ///     fragment_size Each field highlighted can control the size of the highlighted fragment in characters (defaults to 100), and the maximum number of fragments to return (defaults to 5). 
        /// </summary>
        ...
        /// <summary>
        ///     number_of_fragments Each field highlighted can control the size of the highlighted fragment in characters (defaults to 100), and the maximum number of fragments to return (defaults to 5). If the number_of_fragments value is set to 0 then no fragments are produced, instead the whole
        ///     content of the field is returned, and of course it is highlighted. This can be very handy if short texts (like document title or address) need to be highlighted but no fragmentation is required. Note that fragment_size is ignored in this case.
        /// </summary>
        ...
        /// <summary>
        ///     highlight_query It is also possible to highlight against a query other than the search query by setting highlight_query. This is especially useful if you use a rescore query because those are not taken into account by highlighting by default. Elastic does not validate that
        ///     highlight_query contains the search query in any way so it is possible to define it so legitimate query results aren’t highlighted at all. Generally it is better to include the search query in the highlight_query.
        /// </summary>
        ...
        /// <summary>
        ///     matched_fields The Fast Vector Highlighter can combine matches on multiple fields to highlight a single field using matched_fields. This is most intuitive for multifields that analyze the same string in different ways. All matched_fields must have term_vector set to
        ///     with_positions_offsets but only the field to which the matches are combined is loaded so only that field would benefit from having store set to yes.
        /// </summary>
        ...
        /// <summary>
        ///     The type field allows to force a specific highlighter type. This is useful for instance when needing to use the plain highlighter on a field that has term_vectors enabled. The allowed values are: plain, postings and fvh. 
        /// </summary>
        ...
        /// <summary>
        ///     force_source Forces the highlighting to highlight fields based on the source even if fields are stored separately. Defaults to false. 
        /// </summary>
        ...
        /// <summary>
        ///     When using fast-vector-highlighter one can use fragment_offset parameter to control the margin to start highlighting from. In the case where there is no matching fragment to highlight, the default is to not return anything. Instead, we can return a snippet of text from the beginning
        ///     of the field by setting no_match_size (default 0) to the length of the text that you want returned. The actual length may be shorter than specified as it tries to break on a word boundary. When using the postings highlighter it is not possible to control the actual size of the
