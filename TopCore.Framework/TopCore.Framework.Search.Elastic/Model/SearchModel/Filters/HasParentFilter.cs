...
{
    /// <summary>
    ///   The has_parent filter accepts a query and a parent type. The query is executed in the parent document space, which is specified by the parent
    ///   type. This filter returns child documents which associated parents have matched. For the rest has_parent filter has the same options and
    ///   works in the same manner as the has_child filter.
    /// </summary>
