...
{
    /// <summary>
    ///     Example use cases: Suggesting "H5N1" when users search for "bird flu" in text Identifying
    ///     the merchant that is the "common point of compromise" from the transaction history of
    ///     credit card owners reporting loss Suggesting keywords relating to stock symbol $ATI for
    ///     an automated news classifier Spotting the fraudulent doctor who is diagnosing more than
    ///     his fair share of whiplash injuries Spotting the tire manufacturer who has a
    ///     disproportionate number of blow-outs In all these cases the terms being selected are not
    ///     simply the most popular terms in a set. They are the terms that have undergone a
    ///     significant change in popularity measured between a foreground and background set. If the
    ///     term "H5N1" only exists in 5 documents in a 10 million document index and yet is found in
    ///     4 of the 100 documents that make up a user’s search results that is significant and
    ///     probably very relevant to their search. 5/10,000,000 vs 4/100 is a big swing in
    ///     frequency. Single-set analysise In the simplest case, the foreground set of interest is
    ///     the search results matched by a query and the background set used for statistical
    ///     comparisons is the index or indices from which the results were gathered.
    /// </summary>
    public class SignificantTermsBucketAggregation : BaseBucketAggregation
    {
        ...
        /// <summary>
        ///     mutual_information, chi_square, gnd, jlh Mutual information as described in
        ///     "Information Retrieval", Manning et al., Chapter 13.5.1 can be used as significance
        ///     score by adding the parameter "mutual_information": { "include_negatives": true }
        ///     Mutual information does not differentiate between terms that are descriptive for the
        ///     subset or for documents outside the subset. The significant terms therefore can
        ///     contain terms that appear more or less frequent in the subset than outside the
        ///     subset. To filter out the terms that appear less often in the subset than in
        ///     documents outside the subset, include_negatives can be set to false. Per default, the
        ///     assumption is that the documents in the bucket are also contained in the background.
        ///     If instead you defined a custom background filter that represents a different set of
        ///     documents that you want to compare to, set Roughly, mutual_information prefers high
        ///     frequent terms even if they occur also frequently in the background. For example, in
        ///     an analysis of natural language text this might lead to selection of stop words.
        ///     mutual_information is unlikely to select very rare terms like misspellings. gnd
        ///     prefers terms with a high co-occurence and avoids selection of stopwords. It might be
        ///     better suited for synonym detection. However, gnd has a tendency to select very rare
        ///     terms that are, for example, a result of misspelling. chi_square and jlh are somewhat
        ///     in-between. It is hard to say which one of the different heuristics will be the best
        ///     choice as it depends on what the significant terms are used for (see for example
        ///     [Yang and Pedersen, "A Comparative Study on Feature Selection in Text
        ///     Categorization", 1997]
        ///     (http://courses.ischool.berkeley.edu/i256/f06/papers/yang97comparative.pdf) for a
