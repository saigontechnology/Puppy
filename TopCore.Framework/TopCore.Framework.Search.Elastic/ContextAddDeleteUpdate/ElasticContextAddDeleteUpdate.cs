...
namespace TopCore.Framework.Search.Elastic.ContextAddDeleteUpdate
{
	public class ElasticContextAddDeleteUpdate
	{
		...
		public ElasticContextAddDeleteUpdate(ITraceProvider traceProvider, CancellationTokenSource cancellationTokenSource, ElasticSerializerConfiguration elasticSerializerConfiguration, HttpClient client, string connectionString)
		{
		...
		{
			_traceProvider.Trace(TraceEventType.Verbose, "{0}: Save changes to Elastic started", "ElasticContextAddDeleteUpdate");
			...
			try
			{
				...
				using (var serializer = new ElasticSerializer(_traceProvider, _elasticSerializerConfiguration, _saveChangesAndInitMappings))
				{
				...
				_traceProvider.Trace(TraceEventType.Verbose, "{1}: sending bulk request: {0}", serializedEntities, "ElasticContextAddDeleteUpdate");
				_traceProvider.Trace(TraceEventType.Verbose, "{1}: Request HTTP POST uri: {0}", _elasticUrlBatch.AbsoluteUri, "ElasticContextAddDeleteUpdate");
				content.Headers.ContentType = new MediaTypeHeaderValue("application/json");
				resultDetails.RequestUrl = _elasticUrlBatch.OriginalString;
				var response = await _client.PostAsync(_elasticUrlBatch, content, _cancellationTokenSource.Token).ConfigureAwait(true);

				resultDetails.Status = response.StatusCode;
				if (response.StatusCode != HttpStatusCode.OK)
				{
					_traceProvider.Trace(TraceEventType.Warning, "{2}: SaveChangesAsync response status code: {0}, {1}", response.StatusCode, response.ReasonPhrase, "ElasticContextAddDeleteUpdate");
				...
				_traceProvider.Trace(TraceEventType.Verbose, "{1}: response: {0}", responseString, "ElasticContextAddDeleteUpdate");
			...
			{
				_traceProvider.Trace(TraceEventType.Warning, oex, "{1}: Get Request OperationCanceledException: {0}", oex.Message, "ElasticContextAddDeleteUpdate");
		...
		public async Task<ResultDetails<bool>> DeleteIndexAsync<T>(bool allowDeleteForIndex)
		{
			...
			var elasticUrlForIndexDelete = string.Format("{0}/{1}", _connectionString, elasticSearchMapping.GetIndexForType(typeof(T)));
		...
		public async Task<ResultDetails<bool>> DeleteInternalAsync(bool allowDeleteForIndex, Uri uri)
		{
			...
			{
				_traceProvider.Trace(TraceEventType.Error, "{0}: Delete Index, index type is not activated for this context. If you want to activate it, set the AllowDeleteForIndex property of the context", "ElasticContextAddDeleteUpdate");
				throw new ElasticException("ElasticContext: Index, index type is not activated for this context. If you want to activate it, set the AllowDeleteForIndex property of the context");
			}
			_traceProvider.Trace(TraceEventType.Verbose, "{1}: Request to delete complete index for Type: {0}", uri, "ElasticContextAddDeleteUpdate");

			var resultDetails = new ResultDetails<bool> { Status = HttpStatusCode.InternalServerError };
			try
			{
				_traceProvider.Trace(TraceEventType.Warning, "{1}: Request HTTP Delete uri: {0}", uri.AbsoluteUri, "ElasticContextAddDeleteUpdate");
				var response = await _client.DeleteAsync(uri, _cancellationTokenSource.Token).ConfigureAwait(false);

				resultDetails.Status = response.StatusCode;
				if (response.StatusCode != HttpStatusCode.OK)
				{
					_traceProvider.Trace(TraceEventType.Warning, "{2}: Delete Index, index type response status code: {0}, {1}", response.StatusCode, response.ReasonPhrase, "ElasticContextAddDeleteUpdate");
					if (response.StatusCode == HttpStatusCode.BadRequest)
					{
						var errorInfo = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
						resultDetails.Description = errorInfo;
				...
				{
					var responseString = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
					_traceProvider.Trace(TraceEventType.Verbose, "{1}: Delete Index Request response: {0}", responseString, "ElasticContextAddDeleteUpdate");
					resultDetails.Description = responseString;
