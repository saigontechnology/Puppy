...
namespace TopCore.Framework.Search.Elastic
{
		...
		where TOld : class
	{
		...
		/// <summary>
		///     Scan ans scroll settings for the reindex. You should keep this small, otherwise the response request will be too larger. Default is 5 seconds with a size of 500 documents pro shard. 
		/// </summary>
		...
		public ElasticReindex(IndexTypeDescription oldIndexTypeDescription, IndexTypeDescription newIndexTypeDescription, string connectionString)
		{
			...
			elasticMappingResolver.AddElasticSearchMappingForEntityType(typeof(TNew), MappingUtils.GetElasticMapping<TNew>(newIndexTypeDescription));
			elasticMappingResolver.AddElasticSearchMappingForEntityType(typeof(TOld), MappingUtils.GetElasticMapping<TOld>(oldIndexTypeDescription));
		...
		public void Reindex(string jsonContent, Func<TOld, object> getKeyMethod, Func<TOld, TNew> convertMethod, Func<TOld, RoutingDefinition> getRoutingDefinition = null)
		{
			...
			TraceProvider.Trace(TraceEventType.Information, "ElasticReindex: Reindex: Total SearchResult in scan: {0}", result.PayloadResult.Hits.Total);

			int indexProccessed = 0;
			while (result.PayloadResult.Hits.Total > indexProccessed)
			{
				TraceProvider.Trace(TraceEventType.Information, "ElasticReindex: Reindex: creating new documents, indexProccessed: {0} SearchResult: {1}", indexProccessed, result.PayloadResult.Hits.Total);
