...
namespace Puppy.Search.Elastic.Model.SearchModel.Filters
{
    ...
    public class GeoBoundingBoxFilter : IFilter
    {
        ...
        /// <summary>
        ///     type The type of the bounding box execution by default is set to memory, which means
        ///     in memory checks if the doc falls within the bounding box range. In some cases, an
        ///     indexed option will perform faster (but note that the geo_point type must have lat
        ///     and lon indexed in this case). Note, when using the indexed option, multi locations
        ///     per document field are not supported.
        /// </summary>
