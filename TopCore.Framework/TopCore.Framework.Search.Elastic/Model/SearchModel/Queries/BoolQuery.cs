...
{
	/// <summary>
	///     A query that matches documents matching boolean combinations of other queries. The bool query maps to Lucene BooleanQuery. It is built using one or more boolean clauses, each clause with a typed occurrence. { "query":{ "bool" : { "must" : [ { "term" : { "details" : "different" } }, {
	///     "term" : { "details" : "data" } } ], "must_not" : [ { "range" : { "id" : { "from" : 7, "to" : 20 } } } ], "should" : [ { "term" : { "details" : "data" } }, { "term" : { "details" : "alone" } } ], "minimum_should_match" : 1, "boost" : 3.0 } } }
	/// </summary>
	public class BoolQuery : IQuery
    {
	    ...
	    /// <summary>
	    ///     disable_coord The bool query also supports disable_coord parameter (defaults to false). Basically the coord similarity computes a score factor based on the fraction of all query terms that a document contains. 
	    /// </summary>
