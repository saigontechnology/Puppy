...
namespace TopCore.Framework.Search.Elastic.Model.SearchModel.Aggregations
{
	...
	public class TermsBucketAggregation : BaseBucketAggregation
    {
	    ...
	    /// <summary>
	    ///     Deferring calculation of child aggregations For fields with many unique terms and a small number of required results it can be more efficient to delay the calculation of child aggregations until the top parent-level aggs have been pruned. Ordinarily, all branches of the aggregation
	    ///     tree are expanded in one depth-first pass and only then any pruning occurs. In some rare scenarios this can be very wasteful and can hit memory constraints. When using breadth_first mode the set of documents that fall into the uppermost buckets are cached for subsequent replay so
	    ///     there is a memory overhead in doing this which is linear with the number of matching documents. In most requests the volume of buckets generated is smaller than the number of documents that fall into them so the default depth_first collection mode is normally the best bet but
	    ///     occasionally the breadth_first strategy can be significantly more efficient. Currently elastic will always use the depth_first collect_mode unless explicitly instructed to use breadth_first as in the above example. Note that the order parameter can still be used to refer to data from
    ...
    {
	    /// <summary>
	    ///     should only be considered when very few documents match a query. Otherwise the ordinals-based execution modes are significantly faster. By default, map is only used when running an aggregation on scripts, since they don’t have ordinals. 
	    /// </summary>
	    map,
	    ...
	    /// <summary>
	    ///     on the contrary to global_ordinals and global_ordinals_low_cardinality allocates buckets dynamically so memory usage is linear to the number of values of the documents that are part of the aggregation scope. It is used by default in inner aggregations. 
	    /// </summary>
