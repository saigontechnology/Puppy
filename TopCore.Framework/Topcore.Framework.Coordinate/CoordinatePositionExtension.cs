...
namespace Topcore.Framework.Coordinate
{
    public static class CoordinatePositionExtension
    {
        ...
        public static Coordinate GetDerivedPosition(this Coordinate src, double radiusMiles, double bearing)
        {
            ...
            Coordinate result = new Coordinate(lat * CoordinateConst.RadiansToDegrees, lon * CoordinateConst.RadiansToDegrees);
            return result;
        }
        ...
        {
            Coordinate top = GetDerivedPosition(src, radiusMiles, 0);
            Coordinate left = GetDerivedPosition(src, radiusMiles, 270);
            Coordinate topLeft = new Coordinate(left.Latitude, top.Longitude);
            return topLeft;
        }
        ...
        {
            Coordinate bot = GetDerivedPosition(src, radiusMiles, 180);
            Coordinate right = GetDerivedPosition(src, radiusMiles, 90);
            Coordinate botRight = new Coordinate(right.Latitude, bot.Longitude);
            return botRight;
        }
    }
}