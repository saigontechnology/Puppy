...
namespace TopCore.Framework.Search.Elastic.ContextAddDeleteUpdate
{
	public class ElasticContextIndexMapping
	{
		...
		public async Task<ResultDetails<string>> CreateIndexWithMappingAsync<T>(IndexDefinition indexDefinition)
		{
			...
			_traceProvider.Trace(TraceEventType.Verbose, "{0}: CreateIndexWithMappingAsync Elastic started", "ElasticContextIndexMapping");
		...
		public async Task<ResultDetails<string>> CreateIndexAsync(string index, IndexSettings indexSettings, IndexAliases indexAliases, IndexWarmers indexWarmers)
		{
			...
			_traceProvider.Trace(TraceEventType.Verbose, "{0}: CreateIndexAsync Elastic started", "ElasticContextIndexMapping");
			var resultDetails = new ResultDetails<string> { Status = HttpStatusCode.InternalServerError };
		...
		public async Task<ResultDetails<string>> CreateTypeMappingForIndexAsync<T>(MappingDefinition mappingDefinition)
		{
			...
			{
				throw new ElasticException("CreateTypeMappingForIndexAsync: A mapping definition with the parent index is required");
			}
			_traceProvider.Trace(TraceEventType.Verbose, "{0}: CreateTypeMappingForIndex Elastic started", "ElasticContextIndexMapping");
			var resultDetails = new ResultDetails<string> { Status = HttpStatusCode.InternalServerError };
			...
			try
			{
				...
				{
					string index = _elasticSerializerConfiguration.ElasticMappingResolver.GetElasticSearchMapping(entityContextInfo.EntityType).GetIndexForType(entityContextInfo.EntityType);
					MappingUtils.GuardAgainstBadIndexName(index);
		...
		{
			_traceProvider.Trace(TraceEventType.Verbose, "{0}: UpdateIndexSettingsAsync Elastic started", "ElasticContextIndexMapping");
			var resultDetails = new ResultDetails<string> { Status = HttpStatusCode.InternalServerError };
		...
		public async Task<ResultDetails<OptimizeResult>> IndexOptimizeAsync(string index, OptimizeParameters optimizeParameters)
		{
			...
			var elasticUrlForPostRequest = string.Format("{0}/{1}/_optimize{2}", _connectionString, index, optimizeParameters.GetOptimizeParameters());
			var uri = new Uri(elasticUrlForPostRequest);
			...
			try
			{
				...
				if (response.StatusCode != HttpStatusCode.OK)
				{
					if (response.StatusCode == HttpStatusCode.BadRequest)
					{
						...
						throw new ElasticException("IndexOptimizeAsync: HttpStatusCode.BadRequest: RoutingMissingException, adding the parent Id if this is a child item...");
					}
			...
			try
			{
				...
				if (response.StatusCode != HttpStatusCode.OK)
				{
					...
					if (response.StatusCode == HttpStatusCode.BadRequest)
					{
						var errorInfo = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
						resultDetails.Description = errorInfo;
						throw new ElasticException("CloseOpenIndexAsync: HttpStatusCode.BadRequest: RoutingMissingException, adding the parent Id if this is a child item...");
					}
					if (response.StatusCode == HttpStatusCode.NotFound)
					{
