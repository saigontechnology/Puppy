...
namespace Puppy.Search.Elastic.ContentExists
{
    public class Exists
    {
        ...
        public async Task<ResultDetails<bool>> ExistsAsync(Uri uri)
        {
            ...
            try
            {
                ...
                if (response.StatusCode != HttpStatusCode.OK)
                {
                    ...
                    _traceProvider.Trace(TraceEventType.Information, "ExistsAsync:  response status code: {0}, {1}",
            ...
            catch (OperationCanceledException oex)
            {
                _traceProvider.Trace(TraceEventType.Verbose, oex,
                    "ExistsAsync:  Get Request OperationCanceledException: {0}", oex.Message);
                return resultDetails;
            }
        }
    }
