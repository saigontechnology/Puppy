...
{
    /// <summary>
    ///     The common terms query is a modern alternative to stopwords which improves the precision
    ///     and recall of search results (by taking stopwords into account), without sacrificing
    ///     performance. The problem Every term in a query has a cost. A search for "The brown fox"
    ///     requires three term queries, one for each of "the", "brown" and "fox", all of which are
    ///     executed against all documents in the index. The query for "the" is likely to match many
    ///     documents and thus has a much smaller impact on relevance than the other two terms.
    ...
    ///     queries that need to be executed. The problem with this approach is that, while stopwords
    ///     have a small impact on relevance, they are still important. If we remove stopwords, we
    ///     lose precision, (eg we are unable to distinguish between "happy" and "not happy") and we
    ///     lose recall (eg text like "The The" or "To be or not to be" would simply not exist in the
    ///     index). The solution The common terms query divides the query terms into two groups: more
    ///     important (ie low frequency terms) and less important (ie high frequency terms which
    ///     would previously have been stopwords). First it searches for documents which match the
    ///     more important terms. These are the terms which appear in fewer documents and have a
    ///     greater impact on relevance. Then, it executes a second query for the less important
    ///     terms — terms which appear frequently and have a low impact on relevance. But instead of
    ///     calculating the relevance score for all matching documents, it only calculates the _score
    ///     for documents already matched by the first query. In this way the high frequency terms
    ///     can improve the relevance calculation without paying the cost of poor performance. If a
    ///     query consists only of high frequency terms, then a single query is executed as an AND
    ...
    ///     minimum_should_match, in this case a high enough value should probably be used. Terms are
    ///     allocated to the high or low frequency groups based on the cutoff_frequency, which can be
    ///     specified as an absolute frequency ( &gt;=1) or as a relative frequency (0.0 .. 1.0).
    ...
    ///     or "video" will automatically behave as stopwords without the need to maintain a manual list.
    /// </summary>
    public class CommonTermsQuery : IQuery
    {
        ...
        /// <summary>
        ///     minimum_should_match This can be set or the high_freq, low_freq values. If
        ///     minimum_should_match is set, the other values will be ignored
        /// </summary>
